...
自动化部署示例:
    # 完全自动化安装
    ./install_nvidia_toolkit.sh --auto --quiet

    # 中国大陆环境推荐（使用镜像加速）
    ./install_nvidia_toolkit.sh --auto --china-mirror --quiet

    # 强制安装并启用完整验证
    ./install_nvidia_toolkit.sh --auto --force --enable-verification

    # 容器环境部署（跳过所有验证）
    ./install_nvidia_toolkit.sh --auto --disable-verification

    # 安装实验版本
    ./install_nvidia_toolkit.sh --auto --experimental

    # 指定版本安装
    ./install_nvidia_toolkit.sh --auto --version "1.16.0-1"

环境变量支持:
    NVIDIA_TOOLKIT_AUTO=true       等同于 --auto
